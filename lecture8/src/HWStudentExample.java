...
public class HWStudentExample {
	...
	String location;
	int workerAmount = 25;
	boolean bancrupt = false;	void hireNewWorker(int n) {
		if (bancrupt) {
			System.out
		...
		HWStudentExample chocolateFactory = new HWStudentExample();
		chocolateFactory.location = "Liepaja";
		chocolateFactory.status();		chocolateFactory.hireNewWorker(5);
		chocolateFactory.fireWorker(555);
		chocolateFactory.status();		chocolateFactory.bancrupt();
		chocolateFactory.status();		chocolateFactory.fireAll();
